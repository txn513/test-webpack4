<template>
    <section>
        <div class="toolbar el-col el-col-24" style="justify-content: space-between">
            ...
            <div>
                ...
                <span v-if="applyStatus=='待处理'">
                    <el-button type="primary" v-if="isShow" v-on:click="modifyPrice()" :loading="listLoading" >修改回收价</el-button>
                ...
                <span v-if="applyStatus=='已发货'">
                    <el-button type="primary" v-if="" v-on:click="confirmCusGetBtn()" :loading="listLoading" >确认收货</el-button>
                </span>
            </div>
        ...
        <!--同意弹出窗-->
        <el-dialog title="初审通过，同意回收" style="width: 100%" v-model="passFormVisible" :visible.sync="passFormVisible">
            <el-form ref="passForm" :model="passForm" :rules="passFormRules" label-width="100px" @submit.prevent="onSubmit" style="margin:20px;width:60%;min-width:600px;">
                ...
                <el-form-item>
                    ...
                    <el-button type="primary" @click.native="passSubmit('passForm')" >同意回收</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
        <!--拒绝寄卖弹出窗-->
        <el-dialog title="拒绝回收" style="width: 100%" v-model="refuseFormVisible" :visible.sync="refuseFormVisible">
        ...
        <el-dialog title="鉴定通过" style="width: 100%" v-model="appraisalPassFormVisible" :visible.sync="appraisalPassFormVisible">
            <el-form ref="appraisalPassForm" :model="appraisalPassForm" :rules="appraisalPassFormRules" label-width="100px" @submit.prevent="onSubmit" style="margin:20px;width:60%;min-width:600px;">
                ...
                <el-form-item label="建议回收价" prop="salePrice">
        ...
        <!--修改回收价弹出窗-->
        <el-dialog title="修改回收价" style="width: 100%" v-model="modifyVisible" :visible.sync="modifyVisible">
            <el-form ref="modifyForm" :model="modifyForm" :rules="modifyFormRules" label-width="100px" @submit.prevent="onSubmit" style="margin:20px;width:60%;min-width:600px;">
                <el-form-item label="回收价" prop="recyclingPrice">
                    <el-input disabled v-model="modifyForm.recyclingPrice" type="number" style="width: 70%;"></el-input>
                </el-form-item>
                <el-form-item label="修改回收价" prop="modifySalePrice">
...
<script>
    ...
    import {getRecyclingInfoById,passOrRefuseSubmitFun,confirmSubmitFun,closeSubmitFun,appraisalPassSubmitFun,modifySubmitFun,returnSubmitFun,makeMoneySubmitFun,confirmCusGetFun} from '../../../api/api';
    export default {
        data() {
            return {
                baseInfo:[],//基本信息
                ...
                appraisalPassFormRules: {
                    ...
                    salePrice: [
                        { required: true, message: '请输入建议回收价', trigger: 'blur' },
                    ]
                },
                //鉴定不通过弹出框
            ...
            modifySubmit(){
                this.$refs.modifyForm.validate((valid) => {
                    if (valid) {
                        ...
                        modifySubmitFun(this.modifyForm).then((data) => {
                            if (this.GLOBAL.isResonseSuccess(data)) {
