<template>
    <section>
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            ...
            <el-form align="right" :inline="true"  class="demo-form-inline">
                ...
                <el-form-item label="当前状态">
                    <el-select v-model="status" placeholder="当前状态" filterable style="width:140px;">
                        <el-option
                ...
                <el-form-item label="">
                    <el-button type="primary" v-on:click="searchMarketing">查询</el-button>
                </el-form-item>
                <el-form-item label="">
        ...
        <el-table v-loading="listLoading" stripe border  highlight-current-row :data="tableData" style="width: 100%;margin-top: 30px;">
            ...
            <el-table-column prop="marketingTitle" label="活动主题" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column v-if="isAuth('mpys:marketingInfo:update') || isAuth('mpys:marketingInfo:delete')"  fixed="right" label="操作" width="250">
                <template slot-scope="scope">
                    ...
                    <el-button v-if="isAuth('mpys:marketingInfo:update') && scope.row.enableFlag == 1" type="primary"  size="mini" @click="getMarketingGoodsBtn(scope.row)">活动商品</el-button>
                    <el-button v-if="isAuth('mpys:marketingInfo:update') && scope.row.enableFlag == 0" type="primary"  size="mini" @click="editMarketingBtn(scope.row)">编辑</el-button>
        ...
        <el-dialog title="新增/编辑" v-model="addFormVisible" :visible.sync="addFormVisible">
            <el-form :model="addCampaignForm" label-width="100px" :rules="addFormRules" ref="addCampaignForm" class="demo-ruleForm">
                <el-form-item label="有效时间" required>
                    <el-col :span="11">
                        <el-form-item prop="startTime">
                            <el-date-picker type="date" placeholder="选择日期" v-model="addCampaignForm.startTime" style="width: 100%;" format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                        </el-form-item>
                    </el-col>
                    <el-col class="line" :span="1">-</el-col>
                    <el-col :span="11">
                        <el-form-item prop="endTime">
                            <el-date-picker type="date" placeholder="选择日期" v-model="addCampaignForm.endTime" style="width: 100%;"  format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-form-item>
                <el-form-item label="活动图片" prop="marketingImg">
                    <el-upload
                            :action="uploadUrl"
                ...
                <el-form-item label="业务类型" prop="businessId">
                    <template>
                        <select-group-add :num ="addCampaignForm.businessId" :select="isEditSelect" @getresult = 'getEditResultNumber'></select-group-add>
                    </template>
                </el-form-item>
                <el-form-item label="类型" prop="channelType">
                    <el-select v-model="addCampaignForm.channelType" placeholder="请选择">
                        <el-option
                ...
                <el-form-item label="活动标题" prop="marketingTitle">
                    <el-input v-model="addCampaignForm.marketingTitle" ></el-input>
                </el-form-item>
                <el-form-item label="链接" prop="skipUrl">
                    <el-input type="text" v-model="addCampaignForm.skipUrl" ></el-input>
                </el-form-item>
                <el-form-item label="备注" prop="remark">
                    <el-input type="textarea" v-model="addCampaignForm.remark"></el-input>
                </el-form-item>
                <el-form-item label="排序号" prop="sortBy">
                    <el-input v-model="addCampaignForm.sortBy" type="number"  style="width: 10%;"></el-input>
                </el-form-item>
                <el-form-item label="是否启用" prop="enableFlag">
                    <el-switch v-model="addCampaignForm.enableFlag"></el-switch>
                </el-form-item>
            </el-form>
        ...
        <!--关联商品管理界面-->
        <el-dialog title="活动商品" v-model="marketingGoodsVisible" :visible.sync="marketingGoodsVisible">
        ...
        <!--新增关联的商品界面-->
        <el-dialog title="添加活动商品" v-model="addMarketingGoodsVisible" :visible.sync="addMarketingGoodsVisible">
            <el-form  :inline="true"  class="demo-form-inline">
                ...
                <el-form-item label="价格区间" >
                    <el-input-number size="small" v-model="product.minPrice" placeholder="" clearable></el-input-number>至
                    <el-input-number size="small" v-model="product.maxPrice" placeholder="" clearable></el-input-number>
                </el-form-item>
                <el-form-item>
...
<script>
    import {getMarketingList,getMarketingOne,addMarketing,updateMarketing,deleteMarketing,enableMarketing} from '../../api/api';
    ...
    export default {
        ...
        data() {
            return {
                ...
                addFormRules: {
                    ...
                    marketingTitle: [{required: true, message: '请输入活动标题', trigger: 'blur'},
        ...
        methods:{
            ...
            getEditResultNumber(result){
                this.addCampaignForm.businessId = result;
            },
            //业务类型转换
            ...
            getMarketing() {
                ...
                getMarketingList(para).then((res) => {
            ...
            handleAvatarSuccess(res, file) {
                ...
                this.addCampaignForm.marketingImg = res.data.relativePath;
            },
            ...
            //编辑按钮
            editMarketingBtn(row){
                var para={id:row.id};
                getMarketingOne(para).then((res) => {
                    if (this.GLOBAL.isResonseSuccess(res)) {
                        this.addCampaignForm = {
                            id :res.data.id,
            ...
            //添加/编辑信息
            addOrEditSubmit(){
                this.$refs.addCampaignForm.validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            if(this.addCampaignForm.enableFlag){
                                this.addCampaignForm.enableFlag = 1;
                            }else{
                                this.addCampaignForm.enableFlag = 0;
                            }
                            this.addCampaignForm.startTime = util.formatDate.format(new Date(this.addCampaignForm.startTime), 'yyyy-MM-dd hh:mm:ss');
                            this.addCampaignForm.endTime = util.formatDate.format(new Date(this.addCampaignForm.endTime), 'yyyy-MM-dd hh:mm:ss');
                            if(this.addCampaignForm.id== null || this.addCampaignForm.id ==""){
                                //新增
                                addMarketing(this.addCampaignForm).then((res) => {
                                    if (this.GLOBAL.isResonseSuccess(res)) {
                                ...
                                //修改
                                updateMarketing(this.addCampaignForm).then((res) => {
                                    if (this.GLOBAL.isResonseSuccess(res)) {
                                        this.addFormVisible = false;
                                        this.getMarketing();
                                        this.$message({
