<template>
  <section>
    <el-table
      ...
      <el-table-column v-if="isAuth('mpys:menu:update') || isAuth('mpys:menu:delete')" fixed="right" label="操作"  width="260">
        <template slot-scope="scope">
          <el-button v-if="isAuth('mpys:menu:update')" type="primary" size="mini" @click="editStatus(scope.$index, scope.row,$event)">{{scope.row.status == 1 ? "停用" : "启用"}}</el-button>
          <el-button v-if="isAuth('mpys:menu:update')" type="primary" size="mini" @click="handleEdit(scope.$index, scope.row,$event)">编辑</el-button>
          <el-button v-if="isAuth('mpys:menu:delete')" type="primary" size="mini" @click="handleDel(scope.$index, scope.row,$event)">删除</el-button>
        </template>
      </el-table-column>
