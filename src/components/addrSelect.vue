    ...
    export default {
                    ...
                    if(this.address){
                        getListCodeByRegionName({regionNameC:this.address}).then((res) => {
                            ...
                            if (res.code==1) {
                                ...
                                this.editData[0] = res.data[0];
                                this.editData[1] = res.data[1];
                                this.editData[2] = res.data[2];
                            }
                            this.initProvince();
        ...
        methods: {
            ...
            initProvince(){
                getRegion({upperRegion:0}).then((res) => {
                    ...
                    console.log("initProvince"+this.editData.length);
                    //根据code获取其名字
                    if(this.editData.length==3){
                        this.province.value = this.getNameByCode(this.editData[0],res.data);
                        this.initCity();
                    }
                }, (res) => { this.listLoading = false;} ).catch(() => {});
            },
            initCity(){
                ...
                getRegion({upperRegion:param}).then((res) => {
                    ...
                    this.area.value = "";
                    if(!Number(this.province.value)){
                        this.city.value =this.getNameByCode(this.editData[1],res.data);
                        this.initArea();
                    }
                }, (res) => { this.listLoading = false;} ).catch(() => {});
            },
            initArea(){
                ...
                getRegion({upperRegion:param}).then((res) => {
                    ...
                    this.area.value = "";
                    if(!Number(this.city.value)){
                        this.area.value = this.getNameByCode(this.editData[2],res.data);
                    }
                    if(isLoaded) {
                        this.getResult(); //回显使用
                    }
        ...
        mounted() {
            ...
            // console.log("mounted",this.address);
             console.log("mounted",this.editData);
            if(!this.address){
                this.initProvince();
            }
